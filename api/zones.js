...
function create (req, res) {
  ...
  if (!id || !name) {
    return res.status(400).send({ error: true, message: 'Bad Request: id and name are required' })
  }
...
function update (req, res) {
  const { name } = req.body
  if (!name) {
    return res.status(400).send({ error: true, message: 'Bad Request: name is required' })
  }
